...
class Pagination {
  ...
  // PUBLIC
  //----------------------------------------------------------------------
  function __construct($sqlQ, $numRowsPage, $id_page='')
  {
    global $CONFIG_APP;
  ...
  public function getHtmPaginas()
  {
    ...
    if($this->rango[1] > 1) {
      ...
      /** Anterior / Siguiente **/
       ...
       if($this->id_page > 1) {
          ...
          $htmPrev = '<div class="WPaging_PrevNext"><a href="'.$urlPrev.'">&laquo; '.$this->labels['prev'].'</a></div>';
       }
    }
    ...
    if($urlNext = $this->getLinkNext()) {
       $htmNext = '<div class="WPaging_PrevNext"><a href="'.$urlNext.'">'.$this->labels['sig'].' &raquo;</a></div>';
    }

    /** Out **/
     return "
       <table class='obj_Paging'><tr>
         <td>$htmPrev</td><td>&nbsp;</td><td>$htmPaginas</td><td>&nbsp;</td><td>$htmNext</td>
       </tr></table>";
  }
  //----------------------------------------------------------------------
  // PRIVATE
  ...
  private function getUrlParams($id_page) {
    ...
    if($this->hrefSecc == 'FRIENDLY') {
       if($id_page > 1) $ret = 'index_'.$id_page.'.html';
       else             $ret = 'index.html';
    }
    else {
